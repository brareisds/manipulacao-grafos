...
void adiciona_aresta(int id, int u_id, int v_id, grafo G) {
  ...
  // atualiza pesos das arestas incidentes a 'u' e 'v'
  atualiza_pesos(u);
  atualiza_pesos(v);
}
...
void imprime_aresta(aresta e) {
  ...
  int peso_aresta = peso(e);
  printf("%d:{%d,%d,%d}", aresta_id(e), u_id, v_id, peso_aresta);
}

//---------------------------------------------------------
// funcao adicionada para o exercicio 4:

// funcao para atualizar o peso das arestas incidentes a um vértice
void atualiza_pesos(vertice v) {
    no n;
    for (n = primeiro_no(fronteira(v)); n != NULL; n = proximo(n)) {
        aresta a = conteudo(n);
        a->peso = grau(vertice_u(a)) + grau(vertice_v(a)); // recalcula o peso
    }
}